...
ard = serial.Serial('/dev/cu.usbmodem1411',9600) # A PySerial Serial object should be defined outside of this library
maryrgb = {'Ab':(247,75,39), 'A':(255,130,155), 'Bb':(254,123,13), 'B':(254,183,108), 'C':(255,213,0), 'Db':(255,253,175), 'D':(132,255,182), 'Eb':(1,50,252), 'E':(168,227,255), 'F':(225,160,255), 'Gb':(224,215,255), 'G':(60,186,45), 'A#':(254,183,108), 'C#':(255,253,175), 'D#':(1,50,252), 'F#':(225,160,255), 'G#':(247,75,39)} #Mary's note-RGB value synesthesia. leds = np.genfromtxt('hylight_LED_spec.dat', dtype=np.float) #Spectra of the 10 LEDs used in the HyLighter V1.0. All measured at 5% of max duty-cycle, wavelength units are nm. 
ledhues = np.array([310.7591383551636,256.9803013093145,166.34978775550982,139.8502577003946,111.97934945232399,68.0252777377368,50.80611051592259,45.39216459850199,42.398437976796835,7.108444833828318],dtype=np.float) # The CIE LCh hue angle of the LEDs in the HyLight
ledxyzs = np.array([[401.18038167,100.45272655,2325.33593184],[268.12057298,349.16061572,1952.2485356],[90.29998114,739.69145295,271.60012609],[254.91036914,815.20037745,36.91546091],[631.16507486,924.57265065,45.36602422],[128.983218,92.2200947,0.0136668813],[548.498231,267.216229,.00517271580],[97.2855913,44.2955072,0.000409439786],[159.322201,65.5637038,0.000118704440],[1.29825842,0.0502717880,0]],dtype=np.float) # The XYZ values of the LEDs of the HyLight
...
d65XYZ = np.array([38806.58723704,40954.58091754,44049.35046552],dtype=np.float) # XYZ values generated by the HyLight board for it's best D65 reference spectrum
idxcor = [0,7,8,6,5,4,3,2,1,9] # Correct order of the HyLight LEDs on the receiver Arduinos relative to the order of the LED data in this module
...
def send(addr,pwms,power=1.,autoscale=True):
	...
	Args:
		...
		autoscale: bool
			...
			the PWM values are beyond 50% of the max duty-cycle. 

	Returns:
		None
	'''
	pwms=pwms*power # Scale the pwm values by constant value defined by the 'power' variable
	if autoscale and np.any(pwms[0:8]>4096.*.5): # Autoscale the led pwm values to a safe value to not damage the HyLighter boards
		pwms = pwms*(4096.*.5/np.amax(pwms))
	if np.any(pwms[0:8]>4096.*.5): # Check if any LEDs are too bright for the HyLighter board
		raise DangerPWM('PWM value is too large, may damage they LED board: '+str(pwms[np.argwhere(pwms>4096.*.5)]))
