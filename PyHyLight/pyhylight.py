...
leds = np.genfromtxt('hylight_LED_spec.dat', dtype=np.float) #Spectra of the 10 LEDs used in the HyLighter V1.0. All measured at 5% of max duty-cycle, wavelength units are nm. 
...
idxcor = [0,8,7,6,5,4,3,1,2,9] # Correct order of the HyLight LEDs on the receiver Arduinos relative to the order of the LED data in this module
...
def send(addr,pwms,power=1.,autoscale=True):
	...
	if np.any(pwms>4096.*.1): # Check if any LEDs are too bright for the HyLighter board
		raise DangerPWM('PWM value is too large, may damage they LED board: '+str(pwms[np.argwhere(pwms>4096.*.1)]))
