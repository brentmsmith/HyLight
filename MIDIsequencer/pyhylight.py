...
def rgb2hsl(rgb):
	...
	Args:
		rgb: array_like
			The RGB value to be converted into HSL values.
	...
	h = calhue(rgb)
	rgbmax,rgbmin = np.amax(rgb),np.amin(rgb)
	if ((rgb[0]==rgb[1]==b==rgb[2]) or (rgbmax==0)) or ((rgb[0]==rgb[1]==rgb[2]==1) or (rgbmin==1)):
		s=0
	else:
...
def coltemp(T):
	'''
	Converts temperature in K to PWM values to be sent to the HyLight board.
	The function generates a blackbody curve using the Planck function, then
	fits the combined LED spectra XYZ values to the blackbody curve using XYZ
	...
	return np.append(dtycyc.x,0)


def hylrgb(rgb):
	'''
	Adapted from an algorithm designed by Justin Pye (https://github.com/jepye/HyLighter)
...
def send(ard,addr,pwms,power=1.,autoscale=True):
	...
	Args:
		ard: PySerial object (serial.serialposix.Serial)
			The PySerial object initialized for the transmitter Arduino 
			to send commands to be transmitted to the receiver at address addr.
			An example input to this argument will be tran1 where
			tran1 = serial.Serial('/dev/tty.usbmodem1',9600)
		addr: int
		...
		power: float
			...
			all LEDs uniformly by the same constant. Will not exceed 10% of 
			the max duty cycle equal to 409, regardless of power value	
		autoscale: bool
	...
	if not (ard.isOpen()):
		errmsg = "Please set initialize a PySerial object for your transmitter arduino port\ne.g.: tran = serial.Serial('/dev/tty.usbmodem1',9600)\nPerhaps one of the following:\n"+str(listports())
	...
	if np.any(pwms>4096.*.101): # Check if any LEDs are too bright for the HyLighter board
		raise DangerPWM('PWM value is too large, may damage the LED board: '+str(pwms[np.argwhere(pwms>4096.*.101)]))
	if pwms.size!=10: # Check if all the LEDs are being addressed. If not, raise an excepction.
		raise MissingVals('List of PWMs to be sent must contain exactly 10 elements. '+str(pwms.size)+' given.')
